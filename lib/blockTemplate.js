...
var BlockTemplate = module.exports = function BlockTemplate(
    ...
    this.prevHashReversed = util.reverseBuffer(new Buffer.from(rpcData.previousblockhash, 'hex')).toString('hex');
    if (rpcData.finalsaplingroothash) {
        this.hashReserved = util.reverseBuffer(new Buffer.from(rpcData.finalsaplingroothash, 'hex')).toString('hex');
    } else {
    ...
    this.merkleRootReversed = util.reverseBuffer(new Buffer.from(this.merkleRoot, 'hex')).toString('hex');
    ...
    this.getTrees = function (response) {
        ...
        this.hashReserved = util.reverseBuffer(new Buffer.from(response.scTxsCommitment, 'hex')).toString('hex');
        this.merkleRootReversed = util.reverseBuffer(new Buffer.from(this.merkleRoot, 'hex')).toString('hex');
    ...
    this.serializeHeader = function (nTime, nonce) {
        var header = new Buffer.from(140);
        var position = 0;

        header.writeUInt32LE(this.rpcData.version, position += 0, 5, 'hex');
        ...
        header.write(util.reverseBuffer(new Buffer.from(rpcData.bits, 'hex')).toString('hex'), position += 4, 4, 'hex');
    ...
    this.serializeBlock = function (header, soln) {
        ...
        if (this.rpcData.version == 3 && (coin.symbol == "zen" || coin.symbol == "zent")) {
            ...
            if (this.certCount <= 0xfc) {
                var varIntCert = new Buffer.from(certCount, 'hex');
            } else if (this.certCount <= 0x7fff) {
                if (certCount.length == 2) {
                    certCount = "00" + certCount;
                }
                var varIntCert = new Buffer.concat([Buffer('FD', 'hex'), util.reverseBuffer(new Buffer.from(certCount, 'hex'))]);
            }
            tmpBuf = new Buffer.from(varIntCert, 'hex');
    ...
    this.getJobParams = function () {
        if (!this.jobParams) {
            this.jobParams = [
                ...
                util.reverseBuffer(new Buffer.from(this.rpcData.bits, 'hex')).toString('hex'),
                true,
                this.algoNK,
                this.persString
            ];
